...
function generateTableOfContents(chapters: any[]): any[] {
  ...
  function drawTableOfContents(tocEntries: any[]) {
    ...
    tocEntries.forEach(entry => {
      ...
      const pageNumWidth = helveticaFont.widthOfTextAtSize(entry.pageNumber.toString(), fontSize);
      const dotsWidth = maxWidth - textWidth - pageNumWidth - indent;
      
      let line = text;
      if (dotsWidth > 0) {
        const dots = '.'.repeat(Math.floor(dotsWidth / helveticaFont.widthOfTextAtSize('.', fontSize)));
        line += ' ' + dots;
      }
  ...
  data.Chapters.forEach((chapter, index) => {
    ...
    if (addNewPageIfNeeded(80)) return; // Ensure enough space for chapter title and decoration
    ...
    if (chapter.description && chapter.description.blocks.length > 0) {
      const descriptionText = chapter.description.blocks.map((block) =>
        block.text
      ).join(" ");
